#이 코드는 T5 모델의 추론(inference)를 가속화하기 위한 파이썬 클래스의 일부입니다. 주요 함수들은 다음과 같습니다:
# 참고한 사이트: https://discuss.huggingface.co/t/speeding-up-t5-inference/1841
# _encoder_forward(self, input_ids=None, attention_mask=None): 인코더의 순방향 패스를 실행합니다. 입력 ID와 attention mask를 받아서 마지막 hidden state를 반환합니다.
# get_encoder(self): 현재 인스턴스를 반환합니다. 이는 인코더를 가져오는 함수입니다.
# get_output_embeddings(self): 현재 인스턴스를 반환합니다. 이는 출력 임베딩을 가져오는 함수입니다.
# prepare_inputs_for_generation(self, input_ids, past, attention_mask, use_cache, encoder_outputs, **kwargs): 생성을 위한 입력을 준비합니다. 입력 ID, 과거의 hidden states, attention mask, 캐시 사용 여부, 인코더 출력 등을 받아서 딕셔너리 형태로 반환합니다.
# parameters(self): 모델의 파라미터를 반환합니다. 여기서는 단순히 [42, 42] 텐서를 반환하는 예시 코드입니다.
# _export_onnx_graph(self): ONNX 그래프를 내보냅니다. 모델의 이름과 인코더, lm_head의 경로를 사용하여 ONNX 표현을 생성합니다.
# _reorder_cache(self, past, beam_idx): 캐시를 재정렬합니다. 과거의 hidden states와 beam index를 받아서 캐시를 재정렬합니다. 이는 beam search를 사용할 때 유용합니다.
